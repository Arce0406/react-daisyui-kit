...
const ComboInput = forwardRef<HTMLInputElement, ComboInputProps>(({
  ...
  const selectedItems = multiple && Array.isArray(mergedValue) ? mergedValue : [];
  const currentInputValue = !multiple && typeof mergedValue === 'string' ? mergedValue : inputValue;
  ...
  // defaultValue prop 變動時同步 internalValue
  useEffect(() => {
    if (!isControlled) {
